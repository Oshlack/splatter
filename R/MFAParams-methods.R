...
setValidity("MFAParams", function(object) {
    ...
    checks <- c(
        ...
        trans.prop = checkmate::checkNumber(
            v$trans.prop,
            lower = 0,
            upper = 1
        ),
        zero.neg = checkmate::checkLogical(
            v$zero.neg,
            any.missing = FALSE,
            len = 1
        ),
        dropout.present = checkmate::checkLogical(
            v$dropout.present,
            any.missing = FALSE,
            len = 1
        ),
...
setMethod("show", "MFAParams", function(object) {
    pp <- list(
        "Transient:" = c("[Proportion]" = "trans.prop"),
        "Negative:" = c("[Zero]" = "zero.neg"),
        "Dropout:" = c(
            "[Present]" = "dropout.present",
