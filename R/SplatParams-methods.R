...
setValidity("SplatParams", function(object) {
    ...
    checks <- c(
        ...
        batchCells = checkIntegerish(
            v$batchCells,
            lower = 1, len = nBatches
        ),
        batch.facLoc = checkNumeric(v$batch.facLoc, len = nBatches),
        batch.facScale = checkNumeric(
            v$batch.facScale,
            lower = 0, len = nBatches
        ),
        ...
        group.prob = checkNumeric(
            v$de.prob,
            lower = 0, upper = 1, len = nGroups
        ),
        de.prob = checkNumeric(
            v$de.prob,
            lower = 0, upper = 1, len = nGroups
        ),
        de.downProb = checkNumeric(
            v$de.downProb,
            lower = 0, upper = 1, len = nGroups
        ),
        de.facLoc = checkNumeric(v$de.facLoc, len = nGroups),
        de.facScale = checkNumeric(
            v$de.facScale,
            lower = 0, len = nGroups
        ),
        bcv.common = checkNumber(v$bcv.common, lower = 0),
        bcv.df = checkNumber(v$bcv.df, lower = 0),
        dropout.type = checkCharacter(
            v$dropout.type,
            len = 1, any.missing = FALSE
        ),
        dropout.mid = checkNumeric(
            v$dropout.mid,
            finite = TRUE, any.missing = FALSE, min.len = 1
        ),
        dropout.shape = checkNumeric(
            v$dropout.shape,
            finite = TRUE, any.missing = FALSE, min.len = 1
        ),
        path.from = checkIntegerish(
            v$path.from,
            lower = 0, upper = nGroups, len = nGroups
        ),
        path.nSteps = checkIntegerish(
            v$path.nSteps,
            lower = 1, len = nGroups
        ),
        path.skew = checkNumeric(
            v$path.skew,
            lower = 0, upper = 1, len = nGroups
        ),
        path.nonlinearProb = checkNumber(
            v$path.nonlinearProb,
            lower = 0, upper = 1
        ),
    ...
    if (!(v$dropout.type %in%
        ...
        checks <- c(
            checks,
            paste(
                "dropout.type must be one of: 'none', 'experiment',",
                "'batch', 'group', 'cell'"
            )
        )
    }
...
setMethod("setParam", "SplatParams", function(object, name, value) {
    ...
    if (name == "path.length") {
        warning(
            "path.length has been renamed path.nSteps, ",
    ...
    if (name == "dropout.type") {
        ...
        if ((value == "experiment")) {
            if ((mid.len != 1) | (mid.shape != 1)) {
                stop(
                    "dropout.type cannot be set to 'experiment' because ",
        ...
        if ((value == "batch")) {
            ...
            if ((mid.len != n) | (mid.shape != n)) {
                stop(
                    "dropout.type cannot be set to 'batch' because ",
        ...
        if ((value == "group")) {
            ...
            if ((mid.len != n) | (mid.shape != n)) {
                stop(
                    "dropout.type cannot be set to 'group' because ",
                    "dropout.mid and dropout.shape aren't length equal to ",
        ...
        if ((value == "cell")) {
            ...
            if ((mid.len != n) | (mid.shape != n)) {
                stop(
                    "dropout.type cannot be set to 'cell' because ",
                    "dropout.mid and dropout.shape aren't length equal to ",
...
setMethod("expandParams", "SplatParams", function(object) {
    ...
    vectors <- c(
        "de.prob", "de.downProb", "de.facLoc", "de.facScale",
