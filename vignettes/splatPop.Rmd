...
by modeling expression Quantitative Trait Loci (eQTL) effects, where the 
...
(or simulated) genotypes as a vcf file read in as a `VariantAnnotation` object. 
...
those columns, splatPop will function the same as if a GFF was provided. 
However, you can also use this object to specify other information. For example,
if you provide a desired mean (*meanSampled*) and variance (*cvSampled*) for 
each gene, splatPop will use these instead of randomly sampled values. Finally,
if you provide the type (*eQTL.type*, e.g. NA or global), SNP
...
following `splatPopParams` control parameters can be changed using `setParams`:
...
Note that when `splatPopSimulate()` is run, these to objects are contained 
in the output SingleCellExperiment object (details below). Let's look at a 
...
If you already have bulk expression data for a population, 
...
eQTL or group effects, just simulate single-cell counts using the bulk means 
provided. 
...
as group-specific eQTL is set using *eqtl.group.specific*. 

When used to simulate sc data with group-specific effects,`splatSimulatePop` 
also outputs:
