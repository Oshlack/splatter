...
round(sim.means.key$means[1:5, 1:6], digits = 2)
```
...
`splatPopEstimate`. This stage ensures that even if bulk sequencing data are
used to estimate population parameters, the means output from
`splatPopSimulateMeans` will have a distribution resembling single-cell data.

If 
will allows one to simulate population scale single-cell counts
...
sim.sc.gr2 <- splatPopSimulate(vcf=vcf, params=params2)
