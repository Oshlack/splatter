...
be produced using the `mockVCF` function. Genes can be provided in three ways 
(note that options 2 and 3 overwrite nGene in `splatPopParams`): 

1. Randomly (-gff = NA): This option will call `mockGFF` to 
generate a random gff file for a specified chromosome. This is the default
option if neither a gff or key is provided.
2. GFF/GTF (-gff = gff.object): This option takes a GFF/GTF data.frame object
and selects just the genes.
3. Key (-key = key): This option uses the genes in a population key
...
params <- setParams(newSplatPopParams(), "nGenes" = 200, "eqtl.n" = 0.4)
sim.means <- splatPopSimulateMeans(vcf = vcf, params = params)

round(sim.means$means[1:5, 1:6], digits = 3)
```
...
sim.means.gff <- splatPopSimulateMeans(vcf = vcf, params = params, gff=gff)

round(sim.means.gff$means[1:5, 1:6], digits = 3)
```

Here is a snapshot of simulated gene means for genes provided in a splatPop key:

```{r splatPopSimulateMeans-from-key}
partial.key <- sim.means$key[, c("geneID", "chr", "gene_mid")]
sim.means.key <- splatPopSimulateMeans(vcf = vcf, params = params, key=partial.key)

round(sim.means.key$means[1:5, 1:6], digits = 3)
```
...
sim.sc.gr2$sc <- logNormCounts(sim.sc.gr2$sc)
sim.sc.gr2$sc <- runPCA(sim.sc.gr2$sc)
plotPCA(sim.sc.gr2$sc, colour_by = "Group", shape_by = "Sample")
```
