...
splatPop is an extension of the splat model that allows you to simulate single-cell RNA-sequencing count data for a population. Like with splat, these simulations resemble real scRNA-seq data because they use parameters estimated from empirical data. Provided with genotype information (VCF) for a population as input, splatPop simulates gene counts for multiple cells for all individuals in the population. Realistic population structure (the pattern of genetic relatedness between individuals in the population) in the simulations is achieved by modelling expression Quantitative Trait Loci (eQTL) effects, where the expression of a gene is associated with the genotype of the individual at a specific loci. Finally, splatPop allows for the simulation of complex data sets including:
  ...
  1. **Multiple cell groups for each individual:** Where multiple, heterogeneous cell-groups are simulated for each individual, that could represent different cell types or the same cell-type before/after a treatment. Where some genes are assigned cell-group specific differential expression (DE) and/or expression Quantitative Trait Loci (eQTL) effects. 
  2. **Conditional effects between individuals:** Where individuals are assigned to conditional cohorts (e.g. different treatment groups or groups with different disease statuses) and some genes are assigned conditional DE and/or eQTL effects. 
  3. **Batch effects:** Like splat, batch effects are simulated by assigning small batch-specific DE effects to all genes. However, splatPop allows for the simulation of different types of batch effects, such as those resulting from multiplexing sequencing designs. 
...
To get started with splatPop, you need genotype information for the population you want to simulate (i.e. a VCF). Genotype information should be provided as a [VariantAnnotation object](https://bioconductor.org/packages/release/bioc/html/VariantAnnotation.html). A mock VariantAnnotation object can be produced using the `mockVCF()` function. Here we simulate single-cell RNA-sequencing counts for 100 random genes for 6 random samples:
...
The parameters used in splatPop have sensible default values estimated from GTEx data (v7, thyroid tissue). However, they can also be estimated from real data provided by the user using the `splatPopEstimate()`. Gamma distributions describing the population-level gene mean and variance levels can be estimated from real population scale bulk or aggregated single-cell RNA-seq data. and eQTL effect sizes from a gamma distribution derived from real eQTL mapping results. The default parameters were derived from GTEx data (v7, thyroid tissue). However, they can also be estimated from user provided data using `splatPopEstimate()`. You can also provide `splatPopEstimate()` with real single-cell RNA-sequencing data to estimate single-cell parameters as in `splatEstimate()`.
...
Population-scale single-cell RNA-sequencing is typically achieved through
multiplexing, where multiple samples are sequenced together in one batch and 
then separated computationally. Two parameters control how splatPop simulates
data with batch structure: *batchCells* and *batch.size*. The 
*batchCells* parameter is a list containing the number of cells you want to 
simulate for each sample in each batch, where the length of the list is the 
total number of batches to simulate. The *batch.size* parameter controls how 
many samples are included in each batch. If (# batches) x (# samples per batch)
is greater than the number of samples in the VCF file, splatPop will randomly 
assign some samples to >1 batch. Thus, by adjusting these two parameters you can 
...
In the following example, we request two batches (with 20 cells/sample in each) 
with 4 samples in each batch. Given the VCF file contains 6 samples, the 
simulated data will have two samples replicated in both batches. 

```{r simulate-population-with-batch-effects}
params.batches <- newSplatPopParams(nGenes = 100,
                                    similarity.scale = 8,
                                    batchCells = c(1, 1),
                                    batch.size = 4,
                                    batch.facLoc = 0.2,
