...
This vignette describes the SplatPop simulation model and the parameters it uses
in more detail.
...
cell count data for an entire population. Like with splat, these simulations
are able to resemble real single-cell data because they use parameters estimated
from real data. Given genotype information for a population, splatPop simulates
gene mean expression levels that take into account population structure and
allowing for the simulation expression Quantitative Trait Loci (eQTL) effects.
Finally, splatPop allows for the simulate complex datasets with cells from
multiple groups (e.g. cell-types), cells along differentiation trajectories,
and cells from different batches. 


## The splatPop model 

The primary simulation function is `splatPopSimulate`, which runs through the 
two main phases of splatPop:

1. `splatPopSimulateMeans`: the simulation of means for all genes for all
individuals in the population.
2. `splatPopSimulateSC`: the simulation of single-cell counts for all cell for 
...
have double boarders and are shaded by the type of data used (blue = single-cell
...
To get started with splatPop, all you need is genotype information for the
population you want to simulate. Genotype information should be provided as a
...
variance levels are sampled from [gamma distributions][gamma] derived from real
population scale RNA-seq data and eQTL effect sizes from a 
[gamma distribution][gamma] derived from real eQTL mapping results. By default,
these parameters are derived from GTEx (v7, thyroid tissue). However, they can 
...
All parameters needed to for splatPop simulations are stored in a 
`splatPopParams` object. In addition to the compartments in `SplatParams` 
described in detail in the [Splat parameters vignette](splat_params.html), 
...
two pieces of input data: genotypes and genes. Mock genotype and gene data can 
be provided using `mockVCF` and `mockGFF`, respectively. These mock functions 
generate random SNP and gene annotation data for chromosome 22. To simulate
populations with realistic population structure, the user should provide real
(or simulates) genotypes as a vcf file read in as a `VariantAnnotation` object. 

splatPop takes in information about what genes to simulate in three ways: 

1. GFF/GTF (-gff data.frame): Provide a GFF/GTF file as a `data.frame` object. 
splatPop will filter out all non-gene features (3rd column != gene). This 
method uses real gene names and locations, but will randomly assign expression
values and eQTL effects to these genes. 
2. Key (-key data.frame): Provide a `data.frame` object including information 
about genes you want to simulate. This object must include the gene's name
(*geneID*), chromosome (*chromosome*), and location (*geneMiddle*). With just 
those columns, the splatPop simulation will function the same as if a GFF was
provided.  However, you can also use this object to specify other information 
about the genes. If you provide the desired population wide mean (*meanSampled*)
and variance (*cvSampled*), splatPop will use these instead of randomly sampled 
values. Finally, if you provide the type (*eQTL.type*, e.g. NA or global), SNP
identifier (*eSNP.ID*), and effect size (*eQTL.EffectSize*), splatPop will 
simulate gene means with these eQTL associations instead of generating eQTL 
associations randomly. 
...
option if neither a gff or key is provided.

### Control parameters
Other controls for `splatPopSimulateMeans` are held in the `splatPopParams` 
object. This includes distribution parameters that can be estimated from real
data and also control parameters that must be set by the user. The following `splatPopParams` control parameters can be changed using `setParams`:

* **Population parameters** 
    * `similarity.scale` - Scaling factor for the population variance (cv) rate
    parameter. Increasing this scaling factor makes the simulated individuals
...
genetic variant). These parameters are estimated from real single-cell data as described in [splatter](splatter.html).

### Output

The output of `splatPopSimulate()` is a list containing:
...
Let's look at a snapshot of some simulated means and the corresponding key...
...
using the `-key` flag. If you provide `splatPopSimulateMeans` with the key 
...
simulated gene means for each sample to match a [gamma distribution][gamma] 
...
If you already have matrix of bulk expression data for a population, 
you can use this quantile normalization function directly on that data and use
the output as input to `splatPopSimulateSC`. Note that this will not simulate
eQTL or group effects, just simulate single-cell counts given the gene means. 
...
matrix (or list of matricies if nGroups > 1). The user can also provide 
...
`colData` and information about each gene (e.g. location, eQTL/group effects,
mean for each sample) is held in the `rowData`. 
...
params.group <- setParams(newSplatPopParams(), 
                          nGenes = 50,
                          batchCells = 40,
...
relative weight of theese effects. First we can decrease the sample effect by
...
params.batches <- setParams(newSplatPopParams(), 
                            batchCells = c(20, 20),
                            nGenes = 50,
                            similarity.scale = 9,
                            batch.facLoc = 0.3,
...
params.paths <- setParams(newSplatPopParams(), 
                          batchCells = 40,
                          nGenes = 50,
                          similarity.scale = 6,
                          de.facLoc = 0.5,
