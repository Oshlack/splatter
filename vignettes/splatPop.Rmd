...
```{r quick-start}
vcf <- mockVCF(n.samples = 5)

sim <- splatPopSimulate(vcf = vcf, "nGenes" = 100, sparsify = FALSE)
...
params.nCells <- newSplatPopParams(nGenes = 50,
                                   nCells.sample = TRUE,
                                   nCells.shape = 1.1,
                                   nCells.rate = 0.01)

sim.sc.nCells <- splatPopSimulate(vcf = vcf, params = params.nCells, 
                               sparsify = FALSE)

sim.sc.nCells <- logNormCounts(sim.sc.nCells)
sim.sc.nCells <- runPCA(sim.sc.nCells, ncomponents = 10)
...
sim.pop.batches <- splatPopSimulate(vcf = vcf, params = params.batches, sparsify = FALSE)
...
Different distributions can be used to sample batch effect sizes for different batches. For example, here we simulate three batches (5 cells/sample/batch), where batch 1 has a large batch effect compared to batches 2 and 3.
