...
**Modified install instructions for Splatter-eQTL!**
```{r install, eval = FALSE}

#install.packages("~/GitHub/splatter_1.11.1.tar.gz", repos = NULL, type = "source")
#suppressPackageStartupMessages({library(splatter, lib.loc = "/Library/Frameworks/R.framework/Resources/library/")
#'/home/AD.SVI.EDU.AU/cazodi/R/x86_64-pc-linux-gnu-library/3.6/')})
...
## Simulating single cell counts for a population

Simulating single cell counts for multiple individuals/samples can be useful for
functional genomics studies such as expression quantitative trait loci
([eQTL][eQTL]) mapping. For these simulations to be useful they need to capture
realistic patterns of population structure. Splatter uses linkage disequilibrium
(LD) patterns from real/simulated populations and a user defined number of eQTL
associations to simulate normalized mean expression levels for all genes in the
population. The effect sizes of the eQTL associations are simulated from a
[gamma distribution][gamma] derived from real eQTL results. Each gene is then
assigned a mean expression level and coefficient of variation, both sampled from
gamma distributions derived from real expression data. Finally, the mean
expression levels for each individual are quantile normalized to match the gamma
distribution derived from real single cell data by `splatEstiamte`. Finally,
single cell level counts are simulated from these mean expression values using
`splatSimulateeQTL`, which is essentially a wrapper around  `splatSimulate`.


### (Optional) Setting/estimating eQTL-specific parameters

The parameters needed to simulate the population structure aware gene mean
expression levels can be either estimated from real data or defined by the user.
The default parameters were estimated from the GTEx Thyroid data and eQTL
results. These parameters can estimated from other data using `eQTLEstimate` or
be changed manually using `setParams`.

The following parameters are estimated from real data:

* **eQTL.Effect Size**
    * `eqtlES.shape` - Shape parameter for eQTL effect sizes.
    * `eqtlES.rate` - Rate parameter for eQTL effect sizes.
* **eQTL.Mean**
    * `bulkmean.shape` - Shape parameter for mean expression from bulk data.
    * `bulkmean.rate` - Rate parameter for mean expression from bulk data.
    * `bulkcv.param` - Shape and rate parameters for the coefficient of
    variation (cv) across individuals from the bulk dataset, binned by mean
    expression.

```{r eqtlEstimate}
# Set eQTL parameters to default
eqtl.param <- neweQTLParams()

# Adjust eQTL parameters or setting by hand
eqtl.param <- setParams(eqtl.param, 'eqtlES.shape' = 2)
eqtl.param <- setParams(eqtl.param, 'eqtl.n' = 100)

# Estimates eqtl params from real data
data(ex_means)
data(ex_pairs)
eqtl.param <- eQTLEstimate()
eqtl.param
```

### Simulate gene mean expression levels for population

Given a VCF file containing SNP information and a GFF file continaing the genes
to be simulated, `eQTLSimulate` will generate a matrix containing the simulated
count mean for each gene for each individual. This function also saves a
key file that specifies the eSNP:eGene pairs, the effect sizes assigned
to those pairs, and the gene mean and variance assigned to each gene. That key
is saved in a directory called `eqtl_out`.

The `eQTLSimulate` settings and parameters can be altered using `setParams` on
eqt.params object generated by `neweQTLParams` (for default) or `eQTLEstimate`
(using user provided real data). The following parameters are **not estimated**,
but can be changed using `setParams`:

* **Global**
    * `Seed` - Set to ensure reproducible results.
* **eQTL.General**
    * `eqtl.n` - Number of eSNP:eGene pairs to simulate. Default: # genes
    * `eqtl.dist` - Max distance allowed between eSNPs and eGenes. Default: 1 MB
    * `eqtl.maf` - Minor Allele Frequency of SNPs to be considered as possible
    eSNPs. Default: 0.2
    * `eqtl.mafd` - Allowed deviation in MAF (`eqtl.maf` +/- `eqtl.mafd`).
    Default: 0.05
    * `bulkcv.bins` - Number of gene mean bins to use for estimating variance
    parameters. Default: 10

```{r eqtlSimulate}
data(ex_gff)
data(ex_snps)
ex_snps <- ex_snps[, 1:29] # First 20 individuals

eqtl <- eQTLSimulate(eQTLparams = eqtl.param, eqtl.save = FALSE)
round(eqtl[1:5, 1:5], 3)
```

### Simulate single cell data for population
...
column for each cell for each sample, labeled as: "SampleX_CellX".

**NOTE: Working on function to simulate group-specific eQTL... Not done...**

```{r eqtl splatSimulateeQTL}
sim.pop <- splatSimulateeQTL(eqtl = eqtl, batchCells = 200)
...
### Simulate SC data for population with group, path, and/or batch effects

As in the base `splatSimulate` function, users can choose to simulate single
cell data with groups, paths, or batch effects using the `method` tag. Note that
each group/path/batch will have the same population structure and eQTL
associations (i.e. this does not simulate group-specific eQTL data).

```{r eqtl splatSimulate with groups paths and batches}
# Simulate with group effects
params <- newSplatParams()
params <- setParams(params, update = list(de.facLoc=.9))
sim.pop.groups <- splatSimulateeQTL(eqtl = eqtl,
                                    params = params,
                                    method = 'groups',
                                    group.prob = c(0.5, 0.5))
sim.pop.groups <- logNormCounts(sim.pop.groups)
sim.pop.groups <- runPCA(sim.pop.groups)
plotPCA(sim.pop.groups, colour_by = "Sample", shape_by="Group", ncomponents=2)

# Simulate with path effects
sim.pop.paths <- splatSimulateeQTL(eqtl = eqtl,
                                    method = 'paths')
sim.pop.paths <- logNormCounts(sim.pop.paths)
sim.pop.paths <- runPCA(sim.pop.paths)
plotPCA(sim.pop.paths, colour_by = "Step", ncomponents=18:20)

# Simulate with batch effects
sim.pop.batches <- splatSimulateeQTL(eqtl = eqtl,
                                    batchCells = c(50, 50))
sim.pop.batches <- logNormCounts(sim.pop.batches)
sim.pop.batches <- runPCA(sim.pop.batches)
plotPCA(sim.pop.batches, colour_by = "Batch", ncomponents=10:11)

```

### Simulate SC data for population with group-specific eQTL effects

Using the `eQTLSimulate()` function, you can also choose to simulate different
patterns of eSNP-eGene effects using the groups parameter. The output will be a
list of gene mean matrices the length of the number of groups requested. When
provided to `splatSimulateeQTL()`, this allows you to simulate single cell data
where eQTL effects are specific to one group of cells or another.

The degree to which single cells differ by individual vs. differ by group can be
tuned using two parameters:

- `eqtl.n`: controls the number of genes assigned as eGenes. Increasing `eqtl.n`
increases the degree of genetic differences between individuals. Default = 100.
- `eqtl.group.specific`: the proportion of `eqtl.n` eGenes that are cell-group
specific. Increasing  `eqtl.group.specific` increases the degree of genetic
differences between single cell groups. Default = 20%.


```{r group specific eQTL simulations}
params <- newSplatParams()
eqtl.param3 <- neweQTLParams()
eqtl.param3 <- setParam(eqtl.param3, 'eqtl.groups', 3)

eqtl3 <- eQTLSimulate(eQTLparams = eqtl.param3, eqtl.save = TRUE,
                      save.name = 'vignette_eQTL_key.csv')

sim.pop.3 <- splatSimulateeQTL(eqtl = eqtl3, params = params, batchCells = 180)
sim.pop.3 <- logNormCounts(sim.pop.3)
sim.pop.3 <- runPCA(sim.pop.3)
plotPCA(sim.pop.3, colour_by = "Sample", shape_by = "eQTLGroup", ncomponents=2)
```

```{r Show group specific eQTL-effect}
data(ex_snps)
ex_snps <- ex_snps[, 1:29] # First 20 individuals
key <- read.csv('eqtl_out/vignette_eQTL_key.csv', sep=',')
ex_snps[, c('V1', 'V3', 'V4', 'V5', 'V6', 'V7', 'V8', 'V9')] <- NULL
names(ex_snps) <- c('loc', paste0('Sample', 1:(dim(ex_snps)[2]-1)))
ex_snps$loc <- paste0('snp', ex_snps$loc)


# Global SNP: show global eGene:eSNP pair with largest effect size with mean expression > 10
glob_ex <- subset(key, eQTL == 'global' & expMean >= 10)
glob_ex <- glob_ex[with(glob_ex, order(EffectSize_g1, decreasing = TRUE)), ][1, ]
global <- data.frame(sample=colData(sim.pop.3)$Sample,
                     group = colData(sim.pop.3)$eQTLGroup,
                     count = counts(sim.pop.3)[glob_ex$geneID, ])

global_snp <- as.data.frame(t(subset(ex_snps, loc == glob_ex$eSNP_g1)[, 2:(dim(ex_snps)[2])]))
global_snp$sample <- row.names(global_snp)
global2 <- merge(global, global_snp, by = 'sample')
names(global2) <- c('sample', 'group', 'count', 'genotype')
global2$type <- 'global'
glob_effsize <- glob_ex$EffectSize_g1
glob_mean <- glob_ex$expMean

# Group 1 specific: eGene:eSNP g1-specific pair with largest effect size with mean expression > 10
g1_ex <- subset(key, eQTL == 'g1' & expMean >= 10)
g1_ex <- g1_ex[with(g1_ex, order(EffectSize_g1, decreasing = TRUE)), ][1, ]

g1 <- data.frame(sample=colData(sim.pop.3)$Sample,
                 group = colData(sim.pop.3)$eQTLGroup,
                 count = counts(sim.pop.3)[g1_ex$geneID, ])

g1_snp <- as.data.frame(t(subset(ex_snps, loc == g1_ex$eSNP_g1)[, 2:(dim(ex_snps)[2])]))
g1_snp$sample <- row.names(g1_snp)
g1m <- merge(g1, g1_snp, by = 'sample')
names(g1m) <- c('sample', 'group', 'count', 'genotype')

g1m$type <- 'gr1'
g1_effsize <- g1_ex$EffectSize_g1
g1_mean <- g1_ex$expMean

summary(lm(count ~ genotype * group, global2))
summary(lm(count ~ genotype * group, g1m))

both <- rbind(global2, g1m)
both$group <- as.factor(both$group)
ggplot(both, aes(x=genotype, y=log10(count), fill = type)) +
  geom_boxplot(outlier.shape = NA) +
  facet_grid(type ~ group, scales = 'free') + theme_bw()
