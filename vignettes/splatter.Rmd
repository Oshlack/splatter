...
>>>>>>> 5c0787f0d772fe237beabfa089d07e7c1ac0d0ed
```
...
column foreach cell for each sample, labeled as: "SampleX_CellX".
...
list of gene mean matricies the length of the number of groups requested. When
...
eqtl.param2 <- neweQTLParams()
eqtl.param2 <- setParam(eqtl.param2, 'eqtl.groups', 2)

eqtl2 <- eQTLSimulate(eQTLparams = eqtl.param2, eqtl.save = TRUE)

sim.pop.2 <- splatSimulateeQTL(eqtl = eqtl2, params = params,
                               eqtl.group.prop = c(0.4, 0.6),
                               batchCells= 200)
...
key <- read.csv('eqtl_out/20200319_1702_eQTL_key.csv', sep=' ')
# Global SNP: snp42834233 on gene 418
global <- data.frame(sample=colData(sim.pop.2)$Sample,
                     eQTLGroup = colData(sim.pop.2)$eQTLGroup,
                     count = counts(sim.pop.2)['Gene418',])
global_snp <- vcf[,'snp42834233', drop=FALSE]
names(global_snp) <- 'snp'
global_snp$sample <- row.names(global_snp)
global <- merge(global, global_snp, by = 'sample')
global$type <- 'global'

# Group 1 specific: snp42350127 on gene395
gr1 <- data.frame(sample=colData(sim.pop.2)$Sample,
                     eQTLGroup = colData(sim.pop.2)$eQTLGroup,
                     count = counts(sim.pop.2)['Gene395',])
gr1_snp <- vcf[,'snp42350127', drop=FALSE]
names(gr1_snp) <- 'snp'
gr1_snp$sample <- row.names(gr1_snp)
gr1 <- merge(gr1, gr1_snp, by = 'sample')
gr1$type <- 'gr1'

both <- rbind(global, gr1)
both$eQTLGroup <- as.factor(both$eQTLGroup)

ggplot(both, aes(x=snp, y=count)) + geom_violin() + facet_grid(eQTLGroup~type)
