...
gamma distributions derived from real bulk expression data. Then the mean 
...
can be either estimated from real data or defined by the user. The default 
parameters were estimated from the GTEx Thyroid data and the resulting eQTL 
mapping results. You can estimate these parameters from your own bulk and 
eQTL mapping data using `eQTLEstimate` or you can change the parameters manually
using `setParams`.
...
Given a VCF file with genotype information, `eQTLSimulate` will simulate 
count mean for each individual for each gene. Genes can be provided as a GFF/GTF
file (use the -gff tag) or as part of a user provided key (use the -key tag)
that must contain: "geneID", "chr", and "loc", with "loc" representing the 
transcriptional start site of the gene. Expression mean and variance will be 
assigned randomly to each gene from gamma distributions parameterized in 
`eQTLEstimate`. **eqtl.n** genes (eGenes) will have SNPs with minor allele 
frequency (MAF) equal to **eqtl.maf +/- eqtl.mafd** that are no more than 
**eqtl.dist** away from the TSS assigned to them. These eGene-eSNP pairs will be 
randomly assigned an effect size (eSNPs) from a gamma distribution parameterized
in `eQTLEstimate`. By default all eQTL relationships will be considered global, 
but group-specific eQTL can be simulated by setting **eqtl.groups** equal to 2+.
This will make a percent (specified by the **eqtl.group.specific** parameter) 
of **eqtl.n** genes specific to a single group. 

All of the parameters in bold above can be changed using `setParams` on the
eqt.params object generated by `neweQTLParams` (for default) or `eQTLEstimate` 
(for user provided real data). The following parameters are *not estimated*, 
but can be changed using `setParams`:

* **eQTL.General**
    * `eqtl.n` - Number of eSNP:eGene pairs to simulate. (Default: 100)
    * `eqtl.dist` - Max distance allowed between eSNPs and eGenes. (Default:
    1 MB)
    ...
    * `eqtl.groups` - Number of groups to simulate (Default = 1)
    * `eqtl.group.specific` - Percent of `eqtl.n` to make group specific 
    (Default = 20%).
...
The output of `eQTLSimulate()` is an object with two components:
  ...
  * `key` - a dataframe listing all of the genes simulated, their assigned means
  and variances, and, for genes assigned an eSNP, the eSNP ID, effect size, and 
  if the eQTL is global or group specific.
...
eqtl.param <- neweQTLParams()  
eqtl <- eQTLSimulate(eQTLparams = eqtl.param, vcf = ex_snps, gff = ex_gff)
...
data("ex_means")
params <- newSplatParams()
bulk_qnorm <- eqtl.quan.norm.sc(params, ex_means)
print(length(bulk_qnorm$means))
round(bulk_qnorm[1:5, 1:5], 3)
```


### Simulate single cell data for a population with global eQTL effects
...
### Simulate single cell data for a population with global and group-specific eQTL effects

As described above, the `eQTLSimulate()` function also allows you simulate 
group-specific eQTL effects. This function can be tuned using the `eqtl.groups`
and `eqtl.group.specific` parameters described above. When used to simulate sc
data with group-specific eQTL, the `splatSimulateeQTL` function also outputs:

* **Cell information (`colData`)**
    * `eQTLGroup` - The group ID for group-specifc eQTL. 
    
```{r group specific eQTL simulations}
eqtl.param2 <- setParam(neweQTLParams(), 'eqtl.groups', 2)
...
plotPCA(sim.pop.2, colour_by = "eQTLGroup", ncomponents=9:13) 
```
...
associations. Note that the groups defined here are **not** the same as the 
groups with group-specific eQTL. In fact, if you use `eQTLSimulate()` with 
`eqtl.groups` = 2 and `splatSimulateeQTL()` with `method` = 'groups' and 
`group.prob` = c(0.5, 0.5), these groups will be simulated separately, where
you would expect ~50% of your cells with eQTLgroup A eQTL-specific effects to be 
assigned to Group X and the other 50% to be assigned to group Y.
...
plotPCA(sim.pop.groups, colour_by = "Group", ncomponents=5) 
