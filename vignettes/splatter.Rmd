...
eqtl <- eQTLSimulate(eQTLparams = eqtl.param)
round(eqtl$means[1:5, 1:5], 3)
```
```{r show eqtl distributions}
top <- eqtl$key %>% mutate(eQTL = ifelse(is.na(eQTL), 'none', eQTL)) %>%
  group_by(eQTL) %>% top_n(n=1, wt = EffectSize) %>% arrange(eQTL)
top <- top[1: length(unique(top$eQTL)), ]

means.g1 <- data.frame(t(eqtl$means[top$geneID, ]))
ggplot(melt(means.g1), aes(x=value, fill = variable)) + geom_histogram() + theme_bw() +
...
sim.pop <- splatSimulateeQTL(eqtl = eqtl$means, batchCells = 100)
...
sim.pop.groups <- splatSimulateeQTL(eqtl = eqtl$means,
                                    method = 'groups',
                                    group.prob = c(0.5, 0.5))
sim.pop.groups <- logNormCounts(sim.pop.groups)
sim.pop.groups <- runPCA(sim.pop.groups)
plotPCA(sim.pop.groups, colour_by = "Group", ncomponents=4)  #shape_by="Group"
...
eqtl.param<- neweQTLParams()
eqtl.param2 <- setParam(eqtl.param, 'eqtl.groups', 2)
eqtl2 <- eQTLSimulate(eQTLparams = eqtl.param2)

sim.pop.2 <- splatSimulateeQTL(eqtl = eqtl2$means, batchCells = 100)
...
top <- eqtl2$key %>% mutate(eQTL = ifelse(is.na(eQTL), 'none', eQTL)) %>%
  group_by(eQTL) %>% top_n(n=1, wt = EffectSize) %>% arrange(eQTL)
top <- top[1: length(unique(top$eQTL)), ]

means.g1 <- data.frame(t(eqtl2$means[['g1']][top$geneID, ]))
means.g1$type <- 'g1-means'
means.g2 <- data.frame(t(eqtl2$means[['g2']][top$geneID, ]))
means.g2$type <- 'g2-means'
means.both <- rbind(means.g1, means.g2)
means.both.m <- tidyr::pivot_longer(means.both, -type)

ggplot(means.both.m, aes(x=value, fill = name)) + geom_histogram() +
    facet_grid(type ~ name, scales='free') + theme_bw()
```
