...
## Simulating eQTL population

Simulating single cell counts for a population with genetic structure and 
genetic variant effects on expression will enable functional genomics studies 
such as expression quantitative trait loci (eQTL) mapping. Splatter simulates
such data by first simulating gene mean expression levels for each
individual in the population using `eqtlSimulate` and then by simulating single
cell level counts based on the mean expression using `splatSimulateeQTL`, which
is essentially a wrapper around the `splatSimulate` function. 


### (Optional) Setting/estimating eQTL-specific parameters

The parameters needed to simulate the gene mean expression data are either
estimated from real data or defined by the user. The default parameters are from 
the GTEx Thyroid data and eQTL results. These parameters can be changed using 
`setParams` or estimated using `eQTLEstimate`.

```{r eqtlEstimate}
eqtl.param <- neweQTLParams()  # Pulls default eqtl parameters
eqtl.param <- setParams(eqtl.param, 'eqtlES.shape' = 2)  # User set params

data(ex_means)
data(ex_pairs)
eqtl.param <- eQTLEstimate()  # Estimates eqtl params from real data
```
...
count mean for each gene for each individual. This function by default saves a 
...
is saved in "eqtl_out".

The `eQTLSimulate` functions can be altered using `'`setParams` on eqt.params
object generated by `neweQTLParams` (for default) or `eQTLEstimate` (using user
defined real data). The following parameters are not estimated, but can be set
by the user:

* **Global**
    * `Seed` - Seed can be set to ensure reproducible results.
* **eQTL.General**
    * `eqtl.n` - Number of eSNP:eGene pairs, if value if > the number of genes 
    in the gff, eqtl.n will update to the number of genes.
    * `eqtl.dist` - Max distance (in base pairs) to allow between eSNPs and 
    eGenes. Default: 1 MB.
    * `eqtl.maf` - Minor Allele Frequency required of SNPs to be considered as
    possible eSNPs. Default: 0.2.
    * `eqtl.mafd` - Deviation in Minor Allele Frequency required of SNPs to be
    considered as possible eSNPs. Default: 0.05 (i.e. MAF = 0.2 +/- 0.05).
    * `bulkcv.bins` - Number of bins to split gene means into when estimating
    parameters for the distribution of variance for genes in that range. 
    Default: 10.
...
around the `splatSimulate` function. As in the base `splatSimulate` function,
users can choose to simulate data with groups, paths, or batch effects by 
specifying the `method`. The output is a single sce object with a column for 
each cell for each sample, labeled as: "SampleX_CellX".

```{r eqtl splatSimulate}
sim.pop <- splatSimulateeQTL(eqtl = eqtl)
...
sim.pop.groups <- splatSimulateeQTL(eqtl = eqtl.small,
                                    method = 'groups',
