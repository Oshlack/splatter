...
uses parameters estimated by `eQTLEstimate` to simulate gene means for each 
individual for each gene. **eqtl.n** genes (eGenes) will be assigned an eSNP
...
eQTL pair) will be randomly assigned an effect size from a gamma distribution 
parameterized in `eQTLEstimate`. By default all eQTL relationships will be 
...
data(ex_snps)
data(ex_gff)
ex_snps <- ex_snps[, c(1:10, 12, 13, 15, 19, 20)]

eqtl <- eQTLSimulate(eQTLparams = eqtl.params, vcf = ex_snps, gff = ex_gff)
round(eqtl$means[1:5, 1:6], digits = 3)
```
...
plotPCA(sim.pop.2, colour_by = "eQTLGroup", shape_by = "Sample",
        ncomponents = 5:6) 
```
